...
function MagneticCTA({ href, children, secondary = false }: { href: string; children: React.ReactNode; secondary?: boolean }) {
  ...
  return <Link ref={ref} href={href} className={`cinematic-cta ${secondary ? 'is-secondary' : ''}`} onPointerMove={(event) => { const node = ref.current; if (!node || window.matchMedia('(pointer: coarse)').matches) return; const rect = node.getBoundingClientRect(); const dx = event.clientX - rect.left - rect.width / 2; const dy = event.clientY - rect.top - rect.height / 2; node.style.transform = `translate3d(${dx * 0.12}px, ${dy * 0.12}px, 0)`; }} onPointerLeave={() => { if (ref.current) ref.current.style.transform = 'translate3d(0,0,0)'; }}>{children}</Link>;
}
...
export function CinematicMarketing() {
  return <main className="cinematic-page">
    ...
    <section className="story-section message-action" data-section="message-action"><div className="story-container centered-story"><span className="cinematic-kicker">02 / SIGNATURE TRANSITION</span><ScrollTextReveal mode="character" className="message-word">MESSAGE</ScrollTextReveal><div className="signal-line"><span /></div><ScrollTextReveal mode="character" className="action-word">ACTION</ScrollTextReveal><p>HANDLE turns incoming business activity into deliberate movement.</p></div></section>
    ...
    <section id="connections" className="story-section integrations-story" data-section="connections"><div className="story-container integrations-layout"><div><span className="cinematic-kicker">06 / CONNECTION LAYER</span><h2>ONE HANDLE.<br /><em>MANY SYSTEMS.</em></h2><p>Rotate through the stack and bring each connector into the active focus.</p></div><ScrollRotation className="integration-orbit"><div className="integration-core">HANDLE<span>CONNECTED</span></div>{integrations.map((tool, index) => <div className={`integration-token token-${index}`} key={tool}>{tool}</div>)}</ScrollRotation></div></section>

    <section className="story-section spiral-story" data-section="workflow"><div className="story-container"><span className="cinematic-kicker">07 / WORKFLOW SPIRAL</span><h2 className="section-mega-title">A BUSINESS<br /><em>IN MOTION.</em></h2><SpiralScroll className="spiral-stage">{spiralItems.map((item, index) => <article data-spiral-item key={item} className="spiral-card"><span>0{index + 1}</span><b>{item}</b><small>HANDLE EVENT</small></article>)}</SpiralScroll></div></section>

    <HorizontalScroll className="horizontal-showcase" travel={-76}><div data-horizontal-track className="horizontal-track">{['CONNECT', 'UNDERSTAND', 'ACT', 'FOLLOW UP', 'ANALYZE'].map((label, index) => <article key={label} className="horizontal-panel"><span>0{index + 1}</span><b>{label}</b><p>{workflow[index % workflow.length][2]}</p><div className="panel-scan" /></article>)}</div></HorizontalScroll>

    <section className="story-section trust-story" data-section="trust"><div className="story-container trust-layout"><div><span className="cinematic-kicker">09 / HUMAN HANDOFF</span><h2>MOTION<br /><em>SLOWS.</em></h2></div><div className="human-panel"><div className="human-pulse" /><span>HUMAN ACTIVE</span><p>When an important moment needs a person, HANDLE makes that handoff calm, explicit and auditable.</p></div></div></section>

    <section className="story-section product-story" data-section="product"><div className="story-container"><span className="cinematic-kicker">10 / PRODUCT</span><h2>THE ABSTRACT<br /><em>BECOMES REAL.</em></h2><SceneTransition className="product-window"><div className="product-sidebar"><b>HANDLE</b><span>Overview</span><span>Inbox</span><span>Knowledge</span><span>Workflows</span><span>Connections</span><span>Analytics</span></div><div className="product-main"><div className="product-topline">TODAY · 14 ACTIVE FLOWS <i /></div><div className="dashboard-grid"><div className="dash-card large"><small>INBOX</small><strong>24</strong><span>customer threads</span></div><div className="dash-card"><small>RUNS</small><strong>84</strong><span>automations</span></div><div className="dash-card"><small>TIME</small><strong>18.4h</strong><span>saved this week</span></div><div className="dash-card workflow-preview"><small>LIVE WORKFLOW</small><div><span>MESSAGE</span><i /> <span>DECIDE</span><i /> <span>ACT</span></div></div></div></div></SceneTransition></div></section>

    <section className="story-section demo-story" data-section="demo"><div className="story-container demo-layout"><div><span className="cinematic-kicker">11 / TRY HANDLE</span><h2>SHOW ME<br /><em>WHAT IT CAN DO.</em></h2><p>Sandbox simulation. No API required.</p></div><DemoPanel /></div></section>

    <section id="pricing" className="story-section pricing-story" data-section="pricing"><div className="story-container"><span className="cinematic-kicker">12 / PRICING</span><h2>START SMALL.<br /><em>HANDLE MORE.</em></h2><div className="price-grid">{[['STARTER','$9','One business'],['GROWTH','$29','Growing operations'],['PRO','$79','Scale and control']].map(([name, price, copy], index) => <article key={name} className={index === 1 ? 'price-card featured' : 'price-card'}><span>{name}</span><strong>{price}<small>/mo</small></strong><p>{copy}</p><div>✓ Inbox<br />✓ Knowledge<br />✓ Workflows<br />✓ Human handoff</div><Link href="/signup">START FREE ↗</Link></article>)}</div></div></section>

    <section className="final-section" data-section="final"><div className="final-particles" /><motion.div className="final-content" initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.45 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}><span className="cinematic-kicker">13 / HANDLE</span><h2>WE <em>HANDLE</em> IT.</h2><p>Your business has better things to do.</p><div><MagneticCTA href="/signup">START FREE ↗</MagneticCTA><MagneticCTA href="/login" secondary>TRY HANDLE →</MagneticCTA></div></motion.div></section>
